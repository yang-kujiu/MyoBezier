...
void CMyoBezierDlg::Bezier_init()
{
	...
	m_bezier.End_Point_x = 2;
	m_bezier.End_Point_y = 1;
...
{
	m_bezier.End_Point_y = m_bezier.End_Point_y + 0.2;
	if (m_bezier.End_Point_y > 4)
	{
		m_bezier.End_Point_y = 4;
...
//	cys_end.Clear();
//	cys_robot.Clear();
//	//for (t1; t1 < (m_bezier.Sm * 2 + 1); t1++)
//	//{
//	//		CSeries cys0 = (CSeries)m_CTchart_Bezier_path.Series(0);
//	//	t2 = 1 - t1 / (m_bezier.Sm * 2);
//	//	cys0.AddXY(m_bezier.Sm*(m_Yout[0] * pow(1 - t2, 5) + 5 * m_Yout[1] * pow(1 - t2, 4)* t2 + 10 * m_Yout[2] * pow(1 - t2, 3)* pow(t2, 2) + 10 * m_Yout[3] * pow(1 - t2, 2)* pow(t2, 3) + 5 * m_Yout[4] * pow(1 - t2, 1)*pow(t2, 4) + m_Yout[5] * pow(t2, 5)), m_bezier.Sm*(m_Xout[0] * pow(1 - t2, 5) + 5 * m_Xout[1] * pow(1 - t2, 4)* t2 + 10 * m_Xout[2] * pow(1 - t2, 3)* pow(t2, 2) + 10 * m_Xout[3] * pow(1 - t2, 2)* pow(t2, 3) + 5 * m_Xout[4] * pow(1 - t2, 1)*pow(t2, 4) + m_Xout[5] * pow(t2, 5)), NULL, RGB(0, 0, 250));
//	//}
//
//
////	cys_end.AddXY(m_bezier.Sm*m_Yout[5], m_bezier.Sm*m_Xout[5],NULL, RGB(125, 125, 125));
//
//	cys_robot.AddXY(2 - m_robot.y, m_robot.x, NULL, RGB(255, 255, 0));
//	cys_robot_path.AddXY(2 - m_robot.y, m_robot.x, NULL, RGB(255, 0, 0));
}

void CMyoBezierDlg::Get_VandW(double t1)
{
